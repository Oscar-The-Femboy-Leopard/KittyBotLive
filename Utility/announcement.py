...
class Utility(commands.Cog):
    ...
    async def announce(self, ctx, *, message):
        ...
        role = self.bot.get_guild(864654684085682207).get_role(900500138684989460).mention

        # announce_ping = '<@900500138684989460>'

        if msg[0] == ment_ping:
            await self.bot.get_guild(864654684085682207).get_channel(864655320801607700).send(f'{role}\n\n{msg[1]}')
            await ctx.send('Successful!')

        if msg[0] == no_ping:
            await self.bot.get_guild(864654684085682207).get_channel(864655320801607700).send({msg[1]})
            await ctx.send('Successful')
