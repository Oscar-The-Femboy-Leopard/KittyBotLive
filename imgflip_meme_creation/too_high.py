...
aliases = ['toohigh', 'th', 'TooHigh']
description = "Say that something is too high!"
...
class ImgFlip(commands.Cog):
    ...
    async def too_high(self, ctx, *, message):
        content = ctx
        # ctx = content
        id = '61580'
        url = f"https://api.imgflip.com/caption_image?text0={message} is too high!&username={imgflip_username}&password={imgflip_password}&template_id={id}"
        ...
        rm_url = discord.Embed(color=color, title="Things are Too High", timestamp=datetime.datetime.utcnow())
